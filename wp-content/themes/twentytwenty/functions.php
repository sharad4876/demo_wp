...
/* Added on 19 dec 2019 - For fetching data RESTAPI - Sharad */
add_action( 'rest_api_init', 'get_products_data');
function get_products_data() {
            
        register_rest_route( 'my-products', 'getProduct', array(
                'methods' => 'GET',
...
function getProductDetails() {
    ...
    foreach( $posts_list as $posts) {
        $post_id = $posts->ID; // id
        $post_title = $posts->post_title; // name
        $post_content = $posts->post_content; // description
        $post_data[ $post_id ][ 'title' ] = $post_title;
        $post_data[ $post_id ][ 'content' ] = $post_content;
		$post_data[ $post_id ][ 'product_image' ] = get_the_post_thumbnail( $post_id ); // product featured image
		$cusImage = get_field('image',$post_id);		// product extra image
		if( $cusImage ){
			$post_data[ $post_id ][ 'product_extra_image' ] = $cusImage;
		}
    }
    wp_reset_postdata();
